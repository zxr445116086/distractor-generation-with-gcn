...
class DatasetBase(torchtext.data.Dataset):
    ...
    def extract_text_features(tokens, side='tgt', tfidf=None):
        ...
        if side == 'src':
            ...
            dist_out = (1 - pairwise_distances(A, metric="cosine")).tolist()
            dist_out = DatasetBase.directed_graph(dist_out, l1, l2)
            return wordss, dist_out, featuress, n_feats - 1
        else:
            words = []
            features = []
            n_feats = 1
