...
class Trainer(object):
    ...
    def validate(self, valid_iter):
        ...
        for index, batch in enumerate(valid_iter):
            outputs, attns, _, enc_ans_state, enc_last_state = self._forward_prop(batch)

            # Compute loss.
            batch_stats = self.valid_loss.monolithic_compute_loss(
                batch, outputs, attns, enc_ans_state, enc_last_state, index)
    ...
    def _gradient_accumulation(self, batch, normalization, total_stats,
        ...
        outputs, attns, dec_state, enc_ans_state, enc_last_state = self._forward_prop(batch)

        # 3. Compute loss in shards for memory efficiency.
        batch_stats = self.train_loss.compute_loss(
            batch, outputs, attns, normalization, enc_ans_state, enc_last_state)
    ...
    def _forward_prop(self, batch):
        ...
        ques_ans, ques_ans_length = batch.ques_ans
        ans, ans_length = batch.answer
        ...
        outputs, attns, dec_state, enc_ans_state, enc_last_state = \
            self.model(batch.src[0], ques, ans, ques_ans, adj, batch.tgt,
                       batch.src[1], batch.src[2],
                       ques_length, ans_length, ques_ans_length,
                       dec_state)
        return outputs, attns, dec_state, enc_ans_state, enc_last_state
