...
class DGModel(nn.Module):
    ...
    def forward(self, src, ques, ans, ques_ans, adj, tgt,
                sent_lengths, word_lengths, ques_length, ans_length, ques_ans_length,
                dec_state=None):

        tgt = tgt[:-1]  # exclude last target from inputs

        word_mem_bank, sent_mem_bank, quesinit, enc_ans_state, enc_last_state \
            = self.encoder(src, ques, ans, ques_ans, adj, sent_lengths, 
                           word_lengths, ques_length, ans_length, ques_ans_length)
        ...
        return decoder_outputs, attns, dec_state, enc_ans_state, enc_last_state
