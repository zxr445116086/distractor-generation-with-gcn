...
class S2SLossCompute(nn.Module):
    ...
    def compute_loss(self, batch, output, attns, normalization, enc_ans_state, enc_last_state):
        target = batch.tgt[1:]
        s_m = output[-1].squeeze(0)
        v_s = s_m - enc_last_state
        cos = nn.CosineSimilarity(dim=1, eps=1e-6)
        cosine_loss = torch.sum(cos(v_s, enc_last_state))
        bottled_output = self._bottle(output)
        scores = self.generator(bottled_output)
        gtruth = target.view(-1)

        loss = self.criterion(scores, gtruth)
        #loss -= 0.0001 * cosine_loss
        loss.div(float(normalization)).backward()

        stats = self._stats(loss.data.clone(), scores, gtruth)
        return stats

    def monolithic_compute_loss(self, batch, output, attns, enc_ans_state, enc_last_state, index):
        """
        ...
        v_s = s_m - enc_last_state
        cos = nn.CosineSimilarity(dim=1, eps=1e-6)
        cosine_loss = torch.sum(cos(v_s, enc_ans_state))
        bottled_output = self._bottle(output)
        scores = self.generator(bottled_output)
        gtruth = target.view(-1)
        loss = self.criterion(scores, gtruth)
        if index == 0:
            ...
            #print("semantic sim loss:", cosine_loss.item())
